...
def scaleBox_xywh(box_list, scale: Tuple[float, float]) -> np.ndarray:
	box_list[:, 0] *= scale[0]
	box_list[:, 1] *= scale[1]
	box_list[:, 2] *= scale[0]
	box_list[:, 3] *= scale[1]
...
def scaleBox_x1y1x2y2(box_list, scale: Tuple[float, float]) -> np.ndarray:
	box_list[:, 0] *= scale[0]
	box_list[:, 1] *= scale[1]
	box_list[:, 2] *= scale[0]
	box_list[:, 3] *= scale[1]
...
def clipBox(box_list, size: Tuple[int, int]) -> Any:
	box_list = box_list.copy()

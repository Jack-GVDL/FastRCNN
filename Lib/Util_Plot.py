...
from Lib.ModelInfo import TrainResultInfo


# plot confusion matrix
# https://stackoverflow.com/questions/35572000/how-can-i-plot-a-confusion-matrix
def plotConfusionMatrix(info: TrainResultInfo, label_predicted: List[str], label_true: List[str]) -> None:
	...
	sn.heatmap(data_frame, annot=True, fmt="d")
...
def plotAccuracy(info_list: List[List[TrainResultInfo]], label_info: List[str]) -> None:
	line_list: List[Any] = []
...
# the reason of multiple index is for comparison
# the length of label_info should be the same as info_list
def plotLoss(info_list: List[List[TrainResultInfo]], label_info: List[str]) -> None:
	line_list: List[Any] = []
...
if __name__ == '__main__':
	...
	# ----- plot -----
	# plot confusion matrix
	plotConfusionMatrix(info_list_[0][0], ["P1", "P2", "P3"], ["T1", "T2", "T3"])
